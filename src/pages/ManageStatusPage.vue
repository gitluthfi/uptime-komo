            ...
            <div class="shadow-box">
                <template v-if="$root.statusPageListLoaded">
                    ...
                    <a
                        ...
                        :key="statusPage.slug"
                        :href="getAppBasePath() + '/status/' + statusPage.slug"
                        class="item"
                    >
...
export default {
    ...
    methods: {
        ...
        icon(icon) {
            if (icon === "/icon.svg") {
                // Default icon is served by the frontend itself; only needs the
                // base-path prefix (not the dev-mode backend port).
                return getAppBasePath() + icon;
            } else {
