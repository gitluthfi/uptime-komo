        ...
        <div :class="{ edit: enableEditMode }" class="main">
            ...
            <div v-if="hasToken" class="mb-2">
                <div v-if="!enableEditMode">
                    ...
                    <router-link to="/manage-status-page" class="btn btn-primary mb-2">
...
import { getResBaseURL, getAppBasePath } from "../util-frontend";
import {
        ...
        this.getData()
            ...
            .catch(function (error) {
                if (error.response.status === 404) {
                    location.href = getAppBasePath() + "/#/page-not-found";
                }
                console.log(error);
            });
    ...
    methods: {
            ...
            this.$root
                ...
                .emit("saveStatusPage", this.slug, this.config, this.imgDataUrl, this.$root.publicGroupList, (res) => {
                    if (res.ok) {
                        ...
                        setTimeout(() => {
                            this.loading = false;
                            location.href = getAppBasePath() + "/status/" + this.config.slug;
                        }, time);
                    } else {
                        this.loading = false;
                        toast.error(res.msg);
        ...
        deleteStatusPage() {
            this.$root.getSocket().emit("deleteStatusPage", this.slug, (res) => {
                if (res.ok) {
                    this.enableEditMode = false;
                    location.href = getAppBasePath() + "/#/manage-status-page";
                } else {
                    this.$root.toastError(res.msg);
                }
            });
         ...
         */
        discard() {
            location.href = getAppBasePath() + "/status/" + this.slug;
        },
