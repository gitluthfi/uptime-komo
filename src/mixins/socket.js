...
// Public status pages are server-rendered at a real (non-hash) URL and don't need
// an authenticated socket connection.
    ...
    methods: {
        ...
        initSocketIO(bypass = false) {
            ...
            if (!bypass && location.pathname) {
                ...
                // Still sitting at the bare entry route: let Entry.vue decide where to
                // go first (it may redirect to a public status page).
            ...
            if (location.hash === "#/setup-database") {
                return;
            }
            ...
            // Explicit path so the socket connects correctly when this app is
            // deployed under a reverse-proxy subpath (auto-detected, no config needed).
            socket = io(url, {
                path: getAppBasePath() + "/socket.io",
            });
