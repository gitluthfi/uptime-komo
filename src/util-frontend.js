...
// A handful of routes are rendered server-side at a "real" (non-hash) URL,
// so they can have proper title/description/OG tags for link previews and RSS.
// Everything else is served from the same app root and routed client-side via
// the hash, so location.pathname is only ever one of these known suffixes or
// the app root itself.
const SERVER_RENDERED_SUFFIXES = [
    /\/status-page\/?$/,
    /\/status\/[^/]+\/rss\/?$/,
    /\/status\/[^/]+\/?$/,
    /\/status\/?$/,
...
/**
 * Detect the path this app is deployed under (e.g. "/uptime-komo" or "" for root),
 * purely from the current URL, so it works behind a reverse proxy at any subpath
 * without any build-time or runtime configuration.
