 ...
 */
console.log("Welcome to Uptime Komo");
...
log.info("server", "Uptime Komo Version:", checkVersion.version);
...
(async () => {
    ...
    app.get("/", async (request, response) => {
        ...
        if (hostname in StatusPage.domainMappingList) {
            ...
            // Relative (no leading "/") so the browser resolves it against whatever
        ...
        socket.on("setup", async (username, password, callback) => {
            try {
                ...
                if ((await R.knex("user").count("id as count").first()).count !== 0) {
                    throw new Error(
                        "Uptime Komo has been initialized. If you want to run setup again, please delete the database."
                    );
                }
